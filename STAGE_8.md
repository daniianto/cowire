...
- **Color/label editing lives in a small Inspector panel, shown only when exactly one shape is selected.** Consistent with how `SelectionBox` already only renders resize handles for a single selection - editing a shared property across a multi-selection isn't scoped here. As-built: positioned top-right (below the existing button row), not top-left as first built, to make room for the layer tree sidebar; its wrapper `div` is `pointer-events-none` with `pointer-events-auto` on just the color/label controls, since the wrapper otherwise sat on top of the canvas and silently absorbed drags meant to create/move shapes underneath it.
- **As-built fix: canvas keyboard shortcuts now ignore events from text inputs.** `useCanvasInteraction`'s keydown listener is on `window` (needed so shortcuts work regardless of what has focus on the page), which meant every shortcut - Backspace/Delete included - also fired while typing into the Inspector's label field, so fixing a typo could delete the selected shape. Fixed with an early return when `e.target` is an `<input>`/`<textarea>`/`contenteditable` element.
- **As-built addition: an arrow's label gets a background pill.** Unlike a rectangle/circle label (which already sits on the shape's own fill for contrast), an arrow has no fill behind its label - it was unreadable against whatever's on the canvas behind it. `drawCenteredLabel` grew an optional background-color param; arrows pass one, rectangle/circle labels don't (they still rely on `getContrastTextColor` against the shape's own fill).
- **As-built revision: the layer tree is a persistent side panel, not a Dialog.** Originally shipped as a Dialog-triggered panel (see above rationale), but revised at the user's explicit request to always be visible. It's a real flex sibling of the canvas area (`App` becomes a flex row: `LayerTree` then a `flex-1` container holding the canvas and every other overlay) rather than a `position: absolute` layer painted on top - the earlier overlay approach left the sidebar's full-height, real scrollable list physically on top of the canvas, silently swallowing pointer events for any shape drawn/dragged under its footprint (the same class of bug fixed for `Inspector` below, but not fixable the same way here since a real list can't be `pointer-events: none`). Giving it actual layout space means the canvas is laid out narrower to begin with, so this can't recur. Width is a plain fixed Tailwind size (`w-32`, `sm:w-44`), not a collapse toggle - `CLAUDE.md`'s responsiveness requirement is satisfied well enough by staying usable at both sizes, not by hiding the panel.
